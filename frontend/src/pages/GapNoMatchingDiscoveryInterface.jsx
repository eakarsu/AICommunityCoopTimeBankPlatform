...
/**
 * Gap/CFS Feature Page: No matching/discovery interface.
 * Slug: no-matching-discovery-interface
 * Backend: /api/gap-no-matching-discovery-interface
 */
export default function NoMatchingDiscoveryInterfacePage() {
  ...
  async function handleSubmit(e) {
    ...
    try {
      ...
      const res = await fetch('/api/gap-no-matching-discovery-interface', {
        method: 'POST',
        headers: {
  ...
  return (
    <div style={{ padding: 24, maxWidth: 900, margin: '0 auto', color: '#e5e7eb' }}>
      <h2 style={{ marginBottom: 8 }}>No matching/discovery interface.</h2>
      <p style={{ opacity: 0.75, marginBottom: 16 }}>No matching/discovery interface.</p>
