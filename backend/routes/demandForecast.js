/**
 * Custom Feature: demand-forecast
 * Predictive demand forecasting for high-value exchange opportunities
 *
 * POST /api/ai/demand-forecast
 * Auth required. Generated as part of Custom Feature Suggestions scaffold (batch_02).
 * Integration credentials: process.env.FEATURE_DEMAND_FORECAST_KEY
...
const FEATURE_KEY = process.env.FEATURE_DEMAND_FORECAST_KEY;
const SYSTEM_PROMPT = `You are an expert assistant specialized in: Predictive demand forecasting for high-value exchange opportunities.
...
async function callLLM(userPayload) {
  ...
  const response = await fetchFn('https://openrouter.ai/api/v1/chat/completions', {
    ...
    headers: {
      ...
      'X-Title': 'AICommunityCoopTimeBankPlatform - demand-forecast',
    },
    body: JSON.stringify({
...
router.post('/demand-forecast', auth, async (req, res) => {
  try {
    ...
    if (!FEATURE_KEY) res.set('X-Feature-Credentials-Missing', 'FEATURE_DEMAND_FORECAST_KEY');
    const ai = await callLLM({ feature: 'demand-forecast', goal: 'Predictive demand forecasting for high-value exchange opportunities', input: payload });
    try {
      if (pool && pool.query) {
        await pool.query(
          ...
          ['demand-forecast', JSON.stringify(payload), JSON.stringify(ai.result), ai.model, req.user?.id || null]
        );
      }
    } catch (persistErr) {
      console.warn('[demand-forecast] persistence skipped:', persistErr.message);
    }
    return res.json({
      ok: true,
...
router.get('/demand-forecast/health', (req, res) => {
  res.json({
    feature: 'demand-forecast',
    endpoint: '/api/ai/demand-forecast',
